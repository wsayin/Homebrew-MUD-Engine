...
bool event_reset_area( EVENT_DATA *event )
{
   D_AREA *area;
   if( !event )
      return TRUE;
   
   if( ( area = event->owner.dArea ) == NULL )
   {
      bug( "event_reset_area: no area assigned to event." );
      return TRUE;
   }
   
   reset_area( area );
   log_string( "Resetting area: %s", area->name );
   event = alloc_event();
